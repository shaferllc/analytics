...
class EventController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($this->isExcludedIp($website, $request->ip())) {
            return 403;
        }
        ...
        if ($request->input('event')) {
            $this->handleEvent($request, $data);
        } else {
            $this->handlePageview($request, $website, $page, $ua, $data, $date, $time);
        }
    ...
    private function handleEvent(Request $request, array &$data)
    {
        ...
        $data['event'] = implode(':', [$eventName, $eventValue, $eventUnit]);
    }

    private function handlePageview(Request $request, $website, $page, $ua, array &$data, $date, $time)
    {
    ...
    private function addUniqueVisitorData(array &$data, $page, $referrer, $geoData, $browserData, $request, $date, $time)
    {
        ...
        $data['resolution'] = $request->input('screen_resolution');
    ...
    private function parseUrl($url)
    {
        ...
        if (isset($url['host'])) {
            $url['non_www_host'] = preg_replace('/^www\./', '', $url['host']);
            return $url;
        }
