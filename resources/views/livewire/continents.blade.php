<x-site :site="$site">
    <div class="space-y-8">
        <x-breadcrumbs :breadcrumbs="[
            ...
            ['url' => route('sites.analytics.continents', ['site' => $site->id]), 'label' => __('Continents'), 'icon' => 'heroicon-o-globe-alt']
        ]" />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Total Continents') }}</p>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex justify-between text-sm items-center">
                            <span class="text-slate-500 flex items-center gap-2">
                                <x-icon name="heroicon-o-globe-alt" class="w-4 h-4 text-red-500" />
                                {{ __('Single Visit Continents') }}:
                            </span>
                            <span class="font-medium text-red-600 dark:text-red-400">{{ number_format($aggregates['categories_with_single_visit']) }}</span>
                        </div>
                        <div class="flex justify-between text-sm items-center">
                            <span class="text-slate-500 flex items-center gap-2">
                                <x-icon name="heroicon-o-globe-americas" class="w-4 h-4 text-orange-500" />
                                {{ __('Multiple Visit Continents') }}:
                            </span>
            ...
            <div class="space-y-6">
                <div class="relative bg-white/90 dark:bg-slate-800/90 rounded-2xl shadow-lg border border-slate-200/60 dark:border-slate-700/60 p-6 overflow-hidden" x-data="{ isOpen: true }">
                    ...
                    <div x-show="isOpen" x-collapse>
                                ...
                                <div class="relative bg-slate-50/50 dark:bg-slate-700/20 rounded-xl p-6" x-data="{ showDetails: false }">
                                    <div class="flex justify-between items-start mb-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="text-lg font-semibold text-slate-900 dark:text-slate-100">{{ $continent['value'] }}</span>
                                        </div>
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800 dark:bg-emerald-800/30 dark:text-emerald-200">
                                            {{ number_format($continent['unique_visitors']) }} {{ __('Users') }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 gap-4 text-sm mb-4">
                                        <div>
                                            ...
                                            <p class="font-medium text-slate-700 dark:text-slate-300">{{ Carbon\Carbon::parse($continent['first_seen'])->format('M j, Y') }}</p>
                                        </div>
                                        <div>
                                            <p class="text-slate-500 dark:text-slate-400">{{ __('Last Seen') }}</p>
                                            <p class="font-medium text-slate-700 dark:text-slate-300">{{ Carbon\Carbon::parse($continent['last_seen'])->format('M j, Y') }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div x-show="showDetails" x-collapse class="mt-4 space-y-4">
                                        ...
                                        <div class="grid grid-cols-2 gap-4 text-sm">
                                            <div>
                                                ...
                                                <p class="font-medium text-slate-700 dark:text-slate-300">{{ number_format($continent['visit_days']) }}</p>
                                            </div>
                                            <div>
                                                <p class="text-slate-500 dark:text-slate-400">{{ __('Percentage') }}</p>
                                                <p class="font-medium text-slate-700 dark:text-slate-300">{{ number_format($continent['percentage'], 1) }}%</p>
                                            </div>
                                        </div>
                                                ...
                                                <div>
                                                    <div class="flex items-start space-x-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p class="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ number_format($continent['unique_visitor_count']) }}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div>
                                                    <div class="flex items-start space-x-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p class="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ number_format($continent['total_visits']) }}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div>
                                                    <div class="flex items-start space-x-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p class="font-semibold text-slate-800 dark:text-slate-200 mt-0.5">{{ number_format($continent['visits_per_visitor'], 1) }}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <!-- Pagination -->
                        <x-pagination :paginator="$continents" type="compact"/>
                    </div>
                </div>
            </div>
        @else
            <div class="flex flex-col items-center justify-center py-20 bg-gradient-to-br from-slate-50 to-white dark:from-slate-900/90 dark:to-slate-800/90 rounded-2xl border border-dashed border-slate-200/60 dark:border-slate-700/60">
                ...
                <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-2">{{ __('No Continent Data Available') }}</h3>
