@php
    $osData = $this->getOperatingSystemData()['data'];
    $firstOsData =  $this->getOperatingSystemData()['first'];
    $lastOsData = $this->getOperatingSystemData()['last'];
    $aggregates = $this->getOperatingSystemData()['aggregates'];
@endphp
<div x-show="activeTab === 'os'">
    <div class="relative p-6 mb-8 text-indigo-200 bg-gradient-to-br from-indigo-900/40 to-indigo-800/20 rounded-xl border border-indigo-700/50 shadow-lg">
        <div class="absolute -top-3 -left-3">
            <div class="p-3 bg-indigo-600 rounded-lg shadow-lg shadow-indigo-500/30 animate-pulse">
                <x-icon name="heroicon-o-cpu-chip" class="w-6 h-6 text-white" />
            </div>
        </div>
        
        <div class="ml-8">
            <h3 class="font-bold text-xl text-white mb-2 tracking-wide">
                {{ __('Operating system analytics and device metrics') }}
            </h3>
            <p class="text-sm text-indigo-300 leading-relaxed">
                {{ __('OS versions and hardware capabilities, including memory usage, CPU cores, network performance, and device metrics. This data helps understand your visitors\' technical environment.') }}
            </p>
        </div>
    ...
    <div class="space-y-4">
        @foreach($osData as $data)
            @php
                $metrics = [
                    // Hardware
                    //'os' => ['icon' => 'computer-desktop', 'label' => 'Operating System', 'suffix' => ''],
                    'memory' => ['icon' => 'cpu-chip', 'label' => 'Memory', 'suffix' => 'GB'],
                    'cpuCores' => ['icon' => 'circle-stack', 'label' => 'CPU Cores', 'suffix' => ' cores'],
                    //'resolution' => ['icon' => 'rectangle-stack', 'label' => 'Resolutions', 'suffix' => ''],
                    //'memory_usage' => ['icon' => 'cpu-chip', 'label' => 'Memory Usage', 'suffix' => 'MB'],
                    'network_latency' => ['icon' => 'network-signal', 'label' => 'Network Latency', 'suffix' => 'ms'],
                    'connection_speed' => ['icon' => 'signal', 'label' => 'Connection Speed', 'suffix' => ''],
                   // 'page_load_metrics' => ['icon' => 'clock', 'label' => 'Page Load Metrics', 'suffix' => ''],
                    //'battery' => ['icon' => 'battery-50', 'label' => 'Battery', 'suffix' => '%'],
                ];
            @endphp
                    ...
                    <div class="flex items-center gap-6">
                        <div class="flex-shrink-0">
                            <x-tooltip text="{{ ucfirst($data->title) }}">
                                <img src="{{ asset('vendor/analytics/icons/os/' . $data->icon . '.svg') }}" 
