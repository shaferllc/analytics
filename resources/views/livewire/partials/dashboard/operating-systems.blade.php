    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl">
            ...
            @else
                <div class="space-y-4">
                    <div class="flex items-center text-sm font-medium text-gray-500 dark:text-gray-400 border-b border-gray-200 dark:border-gray-700 pb-2">
                        <div class="flex-grow pl-2">{{ __('System') }}</div>
                        ...
                        <div class="group hover:bg-gradient-to-r from-blue-50 to-purple-50 dark:hover:bg-gradient-to-r dark:from-gray-700 dark:to-gray-600 rounded-lg transition-all duration-200 p-2.5 shadow hover:shadow-lg">
                            <div class="flex justify-between items-center mb-1.5">
                                <div class="flex items-center truncate space-x-2 flex-grow">
                                    <div class="relative">
                                        <img src="{{ asset('/vendor/analytics/icons/os/'.formatOperatingSystem($operatingSystem->value)) }}.svg" class="w-5 h-5 rounded-full shadow-sm">
                            ...
                            <div class="bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden shadow-inner">
                                <div class="bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-600 rounded-full h-2 transition-all duration-500 ease-out relative" style="width: {{ $totalVisitors > 0 ? ($operatingSystem->count / $totalVisitors) * 100 : 0 }}%">
                                    <div class="absolute inset-0 bg-white opacity-20 animate-pulse"></div>
                                </div>
                            </div>
                            <div class="mt-0.5 text-xs text-right text-gray-500 dark:text-gray-400 font-medium">
                                {{ $totalVisitors > 0 ? number_format(($operatingSystem->count / $totalVisitors) * 100, 1) : 0 }}% of traffic
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        @if(count($operatingSystems) > 0)
            <div class="p-2 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600">
                <a href="{{ route('analytics.stats.operating_systems', ['website' => $website, 'from' => $range['from'], 'to' => $range['to']]) }}" class="flex items-center justify-center text-white hover:text-yellow-200 font-bold text-lg transition-colors group">
                    <span class="mr-3 uppercase tracking-wider">{{ __('View all :count Operating Systems', ['count' => $totalVisitors]) }}</span>
