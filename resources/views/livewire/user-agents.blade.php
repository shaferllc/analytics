...
<x-loading />
    ...
    <div class="space-y-8">
        <x-breadcrumbs :breadcrumbs="[
            ...
            ['url' => route('sites.analytics.user-agents', ['site' => $site->id]), 'label' => __('User Agents'), 'icon' => 'heroicon-o-globe-alt']
        ]" />
            ...
            <div class="space-y-6">
                <div class="relative bg-white/90 dark:bg-slate-800/90 rounded-2xl shadow-lg border border-slate-200/60 dark:border-slate-700/60 p-6" x-data="{ isOpen: true }">
                   <x-analytics::section-header
                        ...
                        :description="__('Detailed Insights Into User Agents')"
                    ...
                    <div x-show="isOpen" x-collapse>
                            ...
                            @foreach($userAgents as $agent)
                                <div class="relative bg-slate-50/50 dark:bg-slate-700/20 rounded-xl p-6 space-y-6">
                                    <x-analytics::cards.header
                                        ...
                                        :description="$agent['browser_name'] . ' ' . $agent['browser_version']"
                                        :visitors="$agent['unique_visitors']"
                                        :visitors_label="__('Users')"
                                        :color="'blue'"
                                    />

                                    <div class="flex gap-4">
                                        @foreach($this->basicStats($agent, $aggregates) as $stat)
                                    ...
                                    <div class="grid grid-cols-3 gap-6">
                                        ...
                                        <div class="col-span-1 space-y-4">
                                            @foreach($this->visitStats($agent) as $stat)
                                        ...
                                        <div class="col-span-2 bg-white/50 dark:bg-slate-800/50 rounded-lg">
                                            <x-analytics::distribution :distribution="$agent['hourly_distribution']" title="Hourly Distribution" description="The hourly distribution shows the number of visits during each hour of the day."/>
                                        </div>
                                    </div>
                                    ...
                                    <div class="space-y-6">
                                        ...
                                        <div class="bg-white/50 dark:bg-slate-800/50 rounded-lg p-4">
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                @foreach($this->browserInfo($agent) as $section)
                                                    <x-analytics::cards.info :section="$section" :data="$agent" />
                                                @endforeach
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <x-pagination :paginator="$userAgents" type="compact"/>
                    </div>
                </div>
            </div>
        @else
            <div class="flex flex-col items-center justify-center py-20 bg-gradient-to-br from-slate-50 to-white dark:from-slate-900/90 dark:to-slate-800/90 rounded-2xl border border-dashed border-slate-200/60 dark:border-slate-700/60">
                ...
                <h3 class="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-2">{{ __('No User Agent Data Available') }}</h3>
                <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Start Monitoring To Collect User Agent Data') }}</p>
            </div>
        @endif
    </div>
</x-site>
